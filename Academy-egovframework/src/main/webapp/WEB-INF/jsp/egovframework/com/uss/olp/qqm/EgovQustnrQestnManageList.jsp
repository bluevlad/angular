 ...
 * 수정 처리 함수
 ******************************************************** */
function fn_egov_modify_QustnrQestnManage(queId){
	var vFrom = document.listForm;
 ...
 * 상세회면 처리 함수
 ******************************************************** */
function fn_egov_detail_QustnrQestnManage(queId){
	var vFrom = document.listForm;
	vFrom.queId.value = queId;
 ...
 * 문항통계
 ******************************************************** */
function fn_egov_statistics_QustnrQestnManage(queId,queType){
	var vFrom = document.listForm;
	vFrom.queId.value = queId;
	vFrom.queType.value = queType;
...
--%>
<input name="queId" type="hidden" value="">
...
<input name="pageIndex" type="hidden" value="<c:out value='${searchVO.pageIndex}'/>">
	...
	<tr>
		...
		<th><spring:message code="comUssOlpQqm.regist.queCount" /></th><!-- 질문개수 -->
	...
	<c:forEach items="${resultList}" var="resultInfo" varStatus="status">
	  <tr>
		...
		<!-- 질문내용  -->
		<td class="lt_text3L">
			<a href="<c:url value='/uss/olp/qqm/EgovQustnrQestnManageModify.do'/>?queId=${resultInfo.queId}" onClick="fn_egov_modify_QustnrQestnManage('<c:out value="${resultInfo.queId}"/>');return false;"><c:out value='${resultInfo.queTitle}'/></a>
    	</td>
		<!-- 질문유형 -->
		<td class="lt_text3">
			<c:if test="${resultInfo.queType == 'S'}"><spring:message code="comUssOlpQqm.regist.QuestS" /></c:if><!-- 단일선택 -->
			<c:if test="${resultInfo.queType == 'M'}"><spring:message code="comUssOlpQqm.regist.QuestM" /></c:if><!-- 다중선택 -->
    		<c:if test="${resultInfo.queType == 'T'}"><spring:message code="comUssOlpQqm.regist.QuestT" /></c:if><!-- 단답 -->
    		<c:if test="${resultInfo.queType == 'D'}"><spring:message code="comUssOlpQqm.regist.QuestD" /></c:if><!-- 서술 -->
		</td>
		...
		<!-- 통계  -->
	  	<td class="lt_text3">
			...
			<span class="btn_b"><input type="submit" class="btn_submit" style="width:40px;border:solid 0px black;text-align:left;" value="<spring:message code='comUssOlpQqm.value.view'/>" onclick="fn_egov_statistics_QustnrQestnManage('${resultInfo.queId}','${resultInfo.queType}'); return false;"></span><!-- value="보기" -->
			</form>
		</td>
      </tr>	  
	</c:forEach>	
