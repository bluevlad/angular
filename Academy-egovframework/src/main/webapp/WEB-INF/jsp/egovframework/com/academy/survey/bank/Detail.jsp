...
<validator:javascript formName="surveyMst" staticJavascript="false" xhtml="true" cdata="false"/>
 ...
 ******************************************************** */
function fn_save(){
	var varFrom = document.getElementById("surveyMst");

	if(confirm("<spring:message code='common.save.msg' />")){
		varFrom.action =  "<c:url value='/academy/survey/bank/Modify.do' />";
		varFrom.submit();
	}
}
...
<div class="wTableFrm">
<!-- 상단타이틀 -->
<form:form commandName="surveyMst" name="surveyMst" action="" method="post" onSubmit="fn_save(document.forms[0]); return false;">
	<input name="queId" id="queId" type="hidden" value="${resultDetail.queId}">
	
	<c:choose>
	    <c:when test="${resultDetail.queType == 'S' || resultDetail.queType == 'M'}">
	        <input type="hidden" name="tmp" id="tmp" value="${resultDetail.queCount}"/>
	    </c:when>
	    <c:otherwise>
...
<h2>${pageTitle} <spring:message code="title.update" /></h2>
	...
	<tbody >
		...
		<tr>
			...
			<td class="left" colspan="3"><input name="queTitle" type="text" value="${resultDetail.queTitle}"></td>
		</tr>
		<!-- 질문유형 -->	
		...
		<tr>
			...
			<td class="left" colspan="3">
				...
				<option value="${comCodeList.code}" <c:if test="${comCodeList.code eq resultDetail.queType}">selected</c:if>>${comCodeList.codeNm}</option>
				</c:forEach>
				</select>
			</td>
		</tr>
		...
		<tr>
			<th>${title}<span class="pilsu">*</span></th>
			<td class="left" colspan="3">
				<textarea name="queDesc" id="queDesc" class="textarea"  cols="75" rows="10" title="<spring:message code='survey.que.desc'/><spring:message code='input.input'/>" style="width:99%;">${resultDetail.queDesc}</textarea><!-- title="질문내용 입력" -->
			...
			<td class="left" colspan="3">
  				<select id="queCount" name="queCount" title="<spring:message code='survey.queCount'/><spring:message code='input.cSelect'/>" onchange="changeQueuCount();"><!-- title="최대선택건수 선택" -->
			        <c:forEach var="i" begin="1" end="10">
			       	<option value="${i}" <c:if test="${resultDetail.queCount == i}">selected</c:if>>${i}</option>
			        </c:forEach>
		      	</select>
			</td>
		</tr>
        ...
        <c:set var="Sview" value= "none"/>
       	<c:if test="${resultDetail.queCount >= status.count}"><c:set var="Sview" value= ""/></c:if>
        <c:choose>
            <c:when test="${status.count == 1}">
                <c:set var="viw" value = "${resultDetail.queViw1}"/>
            </c:when>
            <c:when test="${status.count == 2}">
                <c:set var="viw" value= "${resultDetail.queViw2}"/>
            </c:when>
            <c:when test="${status.count == 3}">
                <c:set var="viw" value= "${resultDetail.queViw3}"/>
            </c:when>
            <c:when test="${status.count == 4}">
                <c:set var="viw" value= "${resultDetail.queViw4}"/>
            </c:when>
            <c:when test="${status.count == 5}">
                <c:set var="viw" value= "${resultDetail.queViw5}"/>
            </c:when>
            <c:when test="${status.count == 6}">
                <c:set var="viw" value= "${resultDetail.queViw6}"/>
            </c:when>
            <c:when test="${status.count == 7}">
                <c:set var="viw" value= "${resultDetail.queViw7}"/>
            </c:when>
            <c:when test="${status.count == 8}">
                <c:set var="viw" value= "${resultDetail.queViw8}"/>
            </c:when>
            <c:when test="${status.count == 9}">
                <c:set var="viw" value= "${resultDetail.queViw9}"/>
            </c:when>
            <c:when test="${status.count == 10}">
                <c:set var="viw" value= "${resultDetail.queViw10}"/>
            </c:when>
            <c:otherwise>
                <c:set var="viw" value= ""/>
            </c:otherwise>
