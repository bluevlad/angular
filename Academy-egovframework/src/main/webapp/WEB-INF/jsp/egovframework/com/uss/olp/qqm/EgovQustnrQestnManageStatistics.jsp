...
<div class="wTableFrm">
	...
	<tbody >
		...
		<tr>
			...
			<td class="left">
  				<c:out value="${resultList[0].queTitle}"/>
			</td>
		</tr>
		<!-- 질문순번 -->
		...
		<tr>
			<th>${title}<span class="pilsu">*</span></th>
			<td class="left">
  				<c:out value="${resultList[0].queCount}"/>
			</td>
		</tr>
		<!-- 질문유형 -->
		...
		<tr>
			<th>${title}<span class="pilsu">*</span></th>
			<td class="left">
				<c:if test="${resultList[0].queType == 'S'}"><spring:message code="comUssOlpQqm.regist.QuestS" /></c:if><!-- 단일선택 -->
				<c:if test="${resultList[0].queType == 'M'}"><spring:message code="comUssOlpQqm.regist.QuestM" /></c:if><!-- 다중선택 -->
	    		<c:if test="${resultList[0].queType == 'T'}"><spring:message code="comUssOlpQqm.regist.QuestT" /></c:if><!-- 단답 -->
	    		<c:if test="${resultList[0].queType == 'D'}"><spring:message code="comUssOlpQqm.regist.QuestD" /></c:if><!-- 서술 -->
			</td>
		</tr>
		<!-- 질문내용 -->
		...
		<tr>
			<th>${title}<span class="pilsu">*</span></th>
			<td class="left">
  				<c:out value="${fn:replace(resultList[0].queDesc , crlf , '<br/>')}" escapeXml="false" />
			</td>
		</tr>
		<!-- 설문항목 결과 -->
		...
		<tr>
			...
			<c:forEach items="${statisticsList}" var="statisticsList" varStatus="status">
				...
				<li>
					<table>						
						<tr>
							<th colspan="2"><c:out value="${statisticsList.qseq}" />. <c:out value="${statisticsList.queviw}" escapeXml="false" /></th>
						</tr>
						<tr>
							<td>
								<span class="g_bar2">
									<span class="g_org" style="width:${statisticsList.queCnt}px;"></span>
								</span>
							</td>
							<td style="width:50px;">			
								(${statisticsList.quePercent})%
							</td>
						</tr>
					</table>	
				</li>
		...
		<tr>
			...
			<td class="left">
			    ...
			    <c:forEach items="${statisticsList2}" var="statisticsList2" varStatus="status2">
			    <c:if test="${statisticsList2.userAnsw ne ''}">
					<tr>
					<td style="border-bottom:1px solid white;">
					<c:out value="${fn:replace(statisticsList2.userAnsw , crlf , '<br/>')}" escapeXml="false" />
					</td>
					</tr>
				</c:if>
			    </c:forEach>
